...
export async function POST(request: NextRequest) {
  try {
    ...
    const notaFiscal = await prisma.notaFiscal.create({
      data: {
        arquivo: file.name, // Por enquanto, só o nome (implementar upload S3 depois)
    ...
    // Processar em background (ou você pode usar uma fila)
...
async function processarNotaFiscalBackground(notaFiscalId: string, buffer: Buffer, userId: string) {
  try {
    ...
    // Agora fazer o matching dos itens com matérias-primas
